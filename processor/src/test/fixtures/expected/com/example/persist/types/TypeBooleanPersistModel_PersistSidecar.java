...
abstract class TypeBooleanPersistModel_PersistSidecar {
  ...
  void restoreState() {
    ...
    if ( !_appStore.isDisposed() ) {
      ...
      if ( null != state ) {
        final Boolean $prop$_getValue = (Boolean) state.get( Keys.PROPERTY_getValue );
  ...
  void persistState() {
    if ( !_appStore.isDisposed() ) {
      ...
      final boolean $prop$_getValue = _peer.getValue();
      if ( $prop$_getValue ) {
  ...
  private static final class Converters {
    ...
    private static TypeConverter createTypeConverter() {
      ...
      converters.put( "getValue", CONVERTER_boolean );
