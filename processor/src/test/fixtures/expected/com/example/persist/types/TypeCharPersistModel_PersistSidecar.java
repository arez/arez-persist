...
abstract class TypeCharPersistModel_PersistSidecar {
  ...
  void restoreState() {
    ...
    if ( !_appStore.isDisposed() ) {
      ...
      if ( null != state ) {
        final Character $prop$_getValue = (Character) state.get( Keys.PROPERTY_getValue );
  ...
  void persistState() {
    if ( !_appStore.isDisposed() ) {
      ...
      final char $prop$_getValue = _peer.getValue();
      if ( 0 != $prop$_getValue ) {
  ...
  private static final class Converters {
    ...
    private static TypeConverter createTypeConverter() {
      ...
      converters.put( "getValue", CONVERTER_char );
