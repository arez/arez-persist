...
abstract class TypeDoublePersistModel_PersistSidecar {
  ...
  void restoreState() {
    ...
    if ( !_appStore.isDisposed() ) {
      ...
      if ( null != state ) {
        final Double $prop$_getValue = (Double) state.get( Keys.PROPERTY_getValue );
  ...
  void persistState() {
    if ( !_appStore.isDisposed() ) {
      ...
      final double $prop$_getValue = _peer.getValue();
      if ( 0.0 != $prop$_getValue ) {
  ...
  private static final class Converters {
    ...
    private static TypeConverter createTypeConverter() {
      ...
      converters.put( "getValue", CONVERTER_double );
