...
abstract class TypeStringPersistModel_PersistSidecar {
  ...
  void restoreState() {
    ...
    if ( !_appStore.isDisposed() ) {
      ...
      if ( null != state ) {
        final String $prop$_getValue = (String) state.get( Keys.PROPERTY_getValue );
  ...
  void persistState() {
    if ( !_appStore.isDisposed() ) {
      ...
      final String $prop$_getValue = _peer.getValue();
      if ( null != $prop$_getValue ) {
  ...
  private static final class Converters {
    ...
    private static TypeConverter createTypeConverter() {
      ...
      converters.put( "getValue", CONVERTER_java__lang__String );
